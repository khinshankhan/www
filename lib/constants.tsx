/* Colors (tailwind colors but we don't actually use tailwind) */
const yellow500 = "oklch(0.795 0.184 86.047)"
const yellow400 = "oklch(0.852 0.199 91.936)"
const blue400 = "oklch(0.707 0.165 254.624)"
const blue300 = "oklch(0.809 0.105 251.813)"
const red500 = "oklch(0.637 0.237 25.331)"
const red400 = "oklch(0.704 0.191 22.216)"
const green500 = "oklch(0.845 0.143 164.978)"
const green400 = "oklch(0.765 0.177 163.223)"

export const color1_bold = yellow500
export const color2_bold = blue400
export const color3_bold = green500
export const color4_bold = red500
export const color1_base = yellow400
export const color2_base = blue300
export const color3_base = green400
export const color4_base = red400

export const defaultSeed = 44364
